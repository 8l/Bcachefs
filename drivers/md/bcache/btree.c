				 ...
				 struct closure *cl)
{
	...
	mutex_lock(&c->bucket_lock);
retry:
	if (__pop_bucket_set(c, GC_MARK_BTREE, 0, &k.key, 1, cl))
		goto err;
...
{
	if (!(btree_gc_wq = create_singlethread_workqueue("bcache_btree_gc")) ||
	    !(btree_io_wq = create_singlethread_workqueue("bcache_btree_io")))
		return -ENOMEM;
