...
static int bch_xattr_get_handler(struct dentry *dentry, const char *name,
				 void *buffer, size_t size, int type)
{
	if (strcmp(name, "") == 0)
		return -EINVAL;

	return bch_xattr_get(dentry->d_sb->s_fs_info, dentry->d_inode->i_ino,
			     name, buffer, size, type);
}

static int bch_xattr_set_handler(struct dentry *dentry, const char *name,
				 const void *value, size_t size,
				 int flags, int type)
{
	return bch_xattr_set(dentry->d_inode, name, value, size, flags, type);
}
