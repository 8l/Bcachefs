...
static const char *prio_read(struct cache *ca, uint64_t bucket)
{
	struct prio_set *p = ca->disk_buckets;
...
void bch_check_mark_super_slowpath(struct cache_set *c, struct bkey *k, bool meta)
{
	unsigned ptr;
...
struct cache_set *bch_cache_set_open(unsigned minor)
{
	...
	mutex_lock(&bch_register_lock);
	c = bch_cache_set_get(idr_find(&bch_extent_minor, minor));
	mutex_unlock(&bch_register_lock);
