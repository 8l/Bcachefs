	...
	for (i = 0; i < bch_extent_ptrs(k); i++)
		if ((ca = PTR_CACHE(c, k, i))) {
			...
			do {
				...
				bad = (!__gc_will_visit_key(c, b->btree_id, k) &&
				       !g->mark.is_metadata);
...
static void bch_cut_subtract_front(struct cache_set *c, const struct bkey *where,
				   struct bkey *k)
{
	...
	while (KEY_SIZE(insert) &&
		...
		if (!replace_key)
			*done = bkey_cmp(k, insert) < 0 ? *k : *insert;
			 ...
			 !bkey_cmpxchg(c, &b->keys, iter, k, replace_key, insert, done))
			continue;
	...
	for (i = bch_extent_ptrs(k) - 1; i >= 0; --i) {
		...
		if ((ca = PTR_CACHE(c, k, i))) {
			...
			do {
				...
				bad = (!stale &&
				       !__gc_will_visit_key(c, b->btree_id, k) &&
				       (mark.is_metadata ||
					(!mark.dirty_sectors &&
