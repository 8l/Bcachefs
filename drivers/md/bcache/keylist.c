			    ...
			    unsigned maxu64s)
{
	...
	new_keys = kmalloc(sizeof(uint64_t) * newsize, GFP_NOIO);
	...
	if (l->top_p < l->bot_p) {
		...
		memcpy(new_keys,
		       l->start_keys_p,
		       (l->top_p - l->start_keys_p) * sizeof(uint64_t));

		/* Copy @bot up to @end_keys */
		memcpy(new_keys + newsize - (l->end_keys_p - l->bot_p),
		       l->bot_p,
		       (l->end_keys_p - l->bot_p) * sizeof(uint64_t));
		...
		memcpy(new_keys,
		       l->bot_p,
		       (l->top_p - l->bot_p) * sizeof(uint64_t));
