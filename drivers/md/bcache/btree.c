...
static void do_btree_node_write(struct btree *b)
{
	...
	if (!bio_alloc_pages(b->bio, __GFP_NOWARN|GFP_NOWAIT)) {
		...
		bch_submit_bbio_replicas(b->bio, b->c, &k.key, ptrs_to_write, true);
			 ...
			 struct closure *cl)
{
	...
	b = bch_btree_node_alloc(c, NULL, 0, id);
	if (!b)
		return -EINVAL;
			...
			struct bkey *k, unsigned level)
{
	...
	while (IS_ERR(b = bch_btree_node_get(c, &op, k, level, true)))
		if (PTR_ERR(b) == -EAGAIN) {
			closure_sync(&op.cl);
		      ...
		      bool flush)
{
	...
	while (!bch_keylist_empty(insert_keys)) {
		...
		/*
		 * recheck because btree_journal_write_get() might've dropped and
		 * retaken write_lock
		 */
		if (u64s > insert_u64s_remaining(b)) {
			need_split = true;
...
{
	int ret = -EINTR;
	uint64_t btree_ptr = b->key.val[0];
