...
static bool do_journal_discard(struct cache *ca)
{
	...
	case DISCARD_READY:
		...
		bio_init(bio);
		bio->bi_iter.bi_sector	= bucket_to_sector(ca,
...
static bool journal_reclaim(struct cache_set *c, u64 *oldest_seq)
{
	...
	for_each_cache_rcu(ca, c, iter) {
		...
		if (bch_extent_ptrs(&e->k) == CACHE_SET_META_REPLICAS_WANT(&c->sb))
			break;
	...
	__releases(c->journal.lock)
{
	...
	for (i = 0; i < bch_extent_ptrs(&e->k); i++) {
		...
		SET_PTR_OFFSET(&e->v.ptr[i], PTR_OFFSET(&e->v.ptr[i]) + sectors);
