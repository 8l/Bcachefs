...
static void bch_key_sort_fixup(struct btree_iter *iter)
{
	while (iter->used > 1) {
...
const struct btree_keys_ops generic_keys_ops = {
...
static void bch_btree_ptr_sort_fixup(struct btree_iter *iter)
{
	while (iter->used > 1) {
		struct btree_iter_set *top = iter->data, *i = top + 1;
...
const struct btree_keys_ops btree_keys_ops = {
	.sort_cmp	= bch_key_sort_cmp,
	.sort_fixup	= bch_btree_ptr_sort_fixup,
	.insert_fixup	= bch_btree_ptr_insert_fixup,
	.key_invalid	= bch_btree_ptr_invalid,
	.key_bad	= bch_btree_ptr_bad,
	.key_to_text	= bch_bkey_to_text,
...
static void bch_extent_sort_fixup(struct btree_iter *iter)
{
	while (iter->used > 1) {
		struct btree_iter_set *top = iter->data, *i = top + 1;
...
static bool __bch_extent_insert_fixup(struct cache_set *c,
				      struct btree_keys *b,
				      struct bkey *insert,
				      struct btree_iter *iter,
				      struct bkey *replace_key)
{
	...
	while (1) {
		...
		if (replace_key && KEY_SIZE(k)) {
			...
			if (bch_extent_ptrs(replace_key) != bch_extent_ptrs(k))
				goto check_failed;
...
EXPORT_SYMBOL(__bch_extent_insert_fixup);
				    ...
				    struct bkey *replace_key)
{
	...
	return __bch_extent_insert_fixup(c, b, insert, iter, replace_key);
}
...
{
	return (l->ptr[bch_extent_ptrs(l)] + r->ptr[bch_extent_ptrs(r)]) &
		~((uint64_t)1 << 63);
}
...
static bool bch_extent_merge(struct btree_keys *bk, struct bkey *l, struct bkey *r)
{
	...
	if (KEY_CSUM(l)) {
		if (KEY_CSUM(r))
			l->ptr[bch_extent_ptrs(l)] = merge_chksums(l, r);
		else
			SET_KEY_CSUM(l, 0);
	}
...
const struct btree_keys_ops extent_keys_ops = {
