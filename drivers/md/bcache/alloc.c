...
static void pd_controllers_update(struct work_struct *work)
{
	...
	bch_pd_controller_update(&c->foreground_write_pd,
				 min(tier0_can_free,
				     div_u64(tier_size[0] *
					     c->foreground_target_percent, 100)),
				 tier_free[0],
				 -1);
...
static void bch_prio_write(struct cache *ca)
{
	...
	for (i = prio_buckets(ca) - 1; i >= 0; --i) {
		...
		/*
		 * goes here before dropping prio_buckets_lock to guard against it
...
static int wait_buckets_available(struct cache *ca)
{
	...
	while (1) {
		...
		if (ca->inc_gen_needs_gc > ca->free_inc.size) {
			...
			/*
			 * We are going to wait for GC to wake us up, even if bucket
			 * counters tell us enough buckets are available, because
			 * we are actually waiting for GC to rewrite nodes with stale
			 * pointers
			 */
		} else if (buckets_available_cache(ca) >= fifo_free(&ca->free_inc))
			break;
...
static void invalidate_buckets_lru(struct cache *ca)
{
	 ...
	 * by read priority and repeatedly replacing the maximum element until all
...
retry:
	...
	for (i = 0; i < bch_extent_ptrs(&b->key); i++) {
		...
		if (b->sectors_free)
			SET_PTR_OFFSET(&src->v.ptr[i], PTR_OFFSET(&src->v.ptr[i]) + sectors);
				  ...
				  unsigned n_wp)
{
	...
	for (wpno = 0; wpno < n_wp; wpno++, wp++)
		(void) bch_stop_write_point(ca, wp);
