...
static inline uint8_t bucket_gc_gen(struct cache *ca, size_t b)
{
...
{
	struct bucket_mark mark = ACCESS_ONCE(g->mark);
...
static struct open_bucket *bch_open_bucket_get(struct cache_set *c, bool moving_gc,
					       struct closure *cl)
{
...
found:
	...
	if (!b) {
		...
		if (ca->gc_buckets[gen] || dynamic_fault()) {
			/* we raced */
			mutex_unlock(&c->bucket_lock);
			bch_bucket_free_never_used(c, &b->key);
			bch_open_bucket_put(c, b);
