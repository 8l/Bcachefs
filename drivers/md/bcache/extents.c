	...
	case BCH_EXTENT:
		...
		for (i = 0; i < bch_extent_ptrs(&e->k); i++)
			if ((ca = PTR_CACHE(c, &e->v, i))) {
				...
				size_t r = bucket_remainder(ca, PTR_OFFSET(&e->v.ptr[i]));
...
bool bch_cut_front(struct bpos where, struct bkey *k)
{
	...
	if (len)
		...
		case BCH_EXTENT:
			...
			for (i = 0; i < bch_extent_ptrs(&e->k); i++)
				SET_PTR_OFFSET(&e->v.ptr[i], PTR_OFFSET(&e->v.ptr[i]) +
					       e->k.size - len);
			break;
		default:
			break;
			   ...
			   int sectors, bool fail_if_stale)
{
	if (sectors && k->type == BCH_EXTENT) {
		int ret = __bch_add_sectors(b->c, b, k, offset, sectors, fail_if_stale);
		if (ret)
			return ret;

		if (!bkey_extent_cached(k))
			bcache_dev_sectors_dirty_add(b->c, k->p.inode, offset, sectors);
	}
 ...
 * the key merging done in bch_btree_insert_key() - for two mergable keys k, j
...
static bool bch_extent_merge(struct btree_keys *bk, struct bkey *l, struct bkey *r)
{
	...
	case KEY_TYPE_DISCARD:
	case KEY_TYPE_ERROR:
		/* These types are mergable, and no val to check */
		break;
