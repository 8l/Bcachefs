...
static void __update_writeback_rate(struct cached_dev *dc)
{
	...
	bch_pd_controller_update(&dc->writeback_pd, target << 9, dirty << 9, -1);
}
...
static void dirty_init(struct dirty_io *io)
{
	...
	bio->bi_max_vecs	= DIV_ROUND_UP(KEY_SIZE(&io->key), PAGE_SECTORS);
...
static void dirty_endio(struct bio *bio, int error)
{
	...
	if (error) {
		trace_bcache_writeback_error(&io->key, io->bio.bi_rw & WRITE, error);
		io->error = error;
	}
...
static void read_dirty(struct cached_dev *dc)
{
	...
	while (!bch_ratelimit_wait_freezable_stoppable(&dc->writeback_pd.rate, &cl)) {
		...
		while (KEY_SIZE(&tmp.k)) {
			...
			bio_for_each_segment_all(bv, &io->bio, i) {
				bv->bv_page = mempool_alloc(dc->writeback_page_pool,
							    i ? GFP_NOWAIT
							      : GFP_KERNEL);
				if (!bv->bv_page) {
					BUG_ON(!i);
