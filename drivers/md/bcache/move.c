...
int bch_move_data_off_device(struct cache *ca)
{
	...
	for (pass = 0;
		...
		if (last_error_count != 0) {
			pr_notice("pass %u:"
				  " error count = %u, error flags = 0x%x",
			      ...
			      const char *name)
{
	...
	for (pass = 0; (pass < MAX_DATA_OFF_ITER); pass++) {
		...
		for_each_btree_node(&iter, ca->set, id, b, NULL) {
			seen++;
retry:
...
int bch_flag_data_bad(struct cache *ca)
{
	...
	if (!ret && !ret2)
		for_each_btree_key(&iter, ca->set, BTREE_ID_EXTENTS, k, &ZERO_KEY)
