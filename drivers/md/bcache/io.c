...
err:
	if (KEY_CACHED(&op->insert_key)) {
		...
		if (!op->replace)
			__bcache_io_error(op->c, "out of space for write %li wait %i",
					  PTR_ERR(b), op->wait);
		op->error = -ENOSPC;
	}
			   ...
			   struct bch_versions_result *v)
{
	...
	for_each_btree_key_with_holes(&iter, c, BTREE_ID_EXTENTS, k,
				      &KEY(inode, bio->bi_iter.bi_sector, 0)) {
		struct bio *n;
		struct bbio *bbio;
		struct cache *ca;
		...
		BUG_ON(bkey_cmp(k, &KEY(inode, bio->bi_iter.bi_sector, 0)) <= 0);
...
version_error_out:
	bcache_io_error(c, bio, "unable to add version");
error_out:
	bch_btree_iter_unlock(&iter);
	return 0;
}
