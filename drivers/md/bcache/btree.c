	...
	for (;
		...
		for (k = i->start;
			...
			if (bkey_next(k) > bset_bkey_last(i)) {
				btree_node_error(b, "key extends past end of bset");
			  ...
			  struct closure *parent)
{
	...
	switch (bch_btree_insert_keys(b, op, insert_keys, replace_key, parent)) {
				      ...
				      int flags)
{
	...
	while ((k = bch_btree_iter_next_filter(&iter, &b->keys, bkey_deleted))) {
