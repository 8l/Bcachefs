 ...
 */
static void bch_btree_node_iter_fix(struct btree_node_iter *iter, struct bkey *where,
			       struct bkey *new)
{
	struct btree_node_iter_set *set;
	u64 n = KEY_U64s(new);
				...
				struct bkey *where, struct bkey *insert)
{
	...
	while (where != bset_bkey_last(i) &&
	       bkey_cmp(insert, b->ops->is_extents ? &START_KEY(where) : where) > 0)
	...
	if (where != bset_bkey_last(i) &&
		 ...
		 * modify insert since the caller needs to be able to journal the key
		 * that was actually inserted (and it can't just pass us a copy
		 * of insert, since ->insert_fixup() might trim insert if this is a
		 * replace operation)
		 */
					       ...
					       struct bset_tree *start)
{
	...
	for (; start <= bset_tree_last(b); start++) {
		...
		bch_btree_node_iter_push(iter, ret, bset_bkey_last(start->data));
	}
