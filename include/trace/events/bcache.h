...
DECLARE_EVENT_CLASS(page_alloc_fail,
	TP_PROTO(struct cache_set *c, uint64_t size),
	TP_ARGS(c, size),
...
TRACE_EVENT(bcache_add_sectors,
	TP_PROTO(struct cache *ca, struct bkey *k, unsigned i,
		 uint64_t offset, int sectors, bool dirty),
...
TRACE_EVENT(bcache_moving_gc_end,
	TP_PROTO(struct cache *ca, uint64_t sectors_moved, uint64_t keys_moved,
		uint64_t buckets_moved),
	TP_ARGS(ca, sectors_moved, keys_moved, buckets_moved),

	TP_STRUCT__entry(
		__array(char,		uuid,	16	)
		__field(uint64_t,	sectors_moved	)
		__field(uint64_t,	keys_moved	)
		__field(uint64_t,	buckets_moved	)
	),
...
TRACE_EVENT(bcache_tiering_end,
	TP_PROTO(struct cache_set *c, uint64_t sectors_moved,
		uint64_t keys_moved),
