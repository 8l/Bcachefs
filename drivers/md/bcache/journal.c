...
static inline void bch_journal_add_prios(struct cache_set *c, struct jset *j)
{
	...
	j->keys += sizeof(struct jset_keys) / sizeof(uint64_t) + c->sb.nr_in_set;;
}
...
reread:		left = ca->sb.bucket_size - offset;
		...
		while (len) {
			...
			if (got != expect) {
				__bch_cache_error(ca,
					"bad version (got %llu expect %llu) "
					"while reading journal from offset %zu",
			...
			if (got != expect) {
				__bch_cache_error(ca,
					"bad checksum (got %llu expect %llu) "
					"while reading journal from offset %zu",
						  got, expect, bucket + offset);
				goto err;
			}
...
static void journal_reclaim(struct cache_set *c)
{
	...
	for_each_cache_rcu(ca, c, iter) {
		...
		k->val[bch_extent_ptrs(k)] =
			PTR(0, bucket_to_sector(c, journal_bucket(ca, ja->cur_idx)),
			    ca->sb.nr_this_dev);
...
{
	unsigned actual_min = nkeys_min + sizeof(struct jset_keys) / sizeof(u64);
	unsigned actual_max = nkeys_max + sizeof(struct jset_keys) / sizeof(u64);
			 ...
			 unsigned nkeys_min, unsigned nkeys_max)
{
	wait_event(c->journal.wait,
		   __journal_res_get(c, res, nkeys_min, nkeys_max));
}
