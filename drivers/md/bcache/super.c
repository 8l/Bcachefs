...
int bch_cached_dev_attach(struct cached_dev *dc, struct cache_set *c)
{
	...
	if (!found) {
		...
		memcpy(dc->disk.inode.i_uuid, dc->sb.uuid, 16);
...
static int flash_dev_run(struct cache_set *c, struct bch_inode_blockdev *inode)
{
	...
	if (bcache_device_init(d, block_bytes(c), inode->i_inode.i_sectors))
		goto err;
...
int bch_flash_dev_create(struct cache_set *c, uint64_t size)
{
	...
	get_random_bytes(inode.i_uuid, 16);
	inode.i_inode.i_ctime = rtime;
	inode.i_inode.i_mtime = rtime;
	inode.i_inode.i_sectors = size >> 9;
...
static void cache_set_free(struct closure *cl)
{
	...
	free_pages((unsigned long) c->sort.sort, c->sort.page_order);
